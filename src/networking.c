...
maqueClient *createClient(int fd) {
    maqueClient *c = zmalloc(sizeof(maqueClient));
    c->bufpos = 0;
...
static void acceptCommonHandler(int fd) {
    ...
    if ((c = createClient(fd)) == NULL) {
        maqueLog(MAQUE_WARNING,"Error allocating resoures for the client");
...
void acceptTcpHandler(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    MAQUE_NOTUSED(el);
    MAQUE_NOTUSED(mask);
    MAQUE_NOTUSED(privdata);

    cfd = anetTcpAccept(server.neterr, fd, cip, &cport);
    if (cfd == AE_ERR) {
        maqueLog(MAQUE_WARNING,"Accepting client connection: %s", server.neterr);
        return;
    }
    maqueLog(MAQUE_VERBOSE,"Accepted %s:%d", cip, cport);
...
void acceptUnixHandler(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    maqueLog(MAQUE_VERBOSE,"Accepted connection to %s", config->unixsocket);
    acceptCommonHandler(cfd);
}
...
void freeClient(maqueClient *c) {
    maqueLog(MAQUE_NOTICE, "调用了 freeClient 函数, 释放客户端资源");
}
...
void readQueryFromClient(aeEventLoop *el, int fd, void *privdata, int mask) {
    maqueClient *c = (maqueClient*) privdata;
    char buf[MAQUE_IOBUF_LEN];
    int nread;
    MAQUE_NOTUSED(el);
    MAQUE_NOTUSED(mask);

    nread = read(fd, buf, MAQUE_IOBUF_LEN);
    if (nread == -1) {
        if (errno == EAGAIN) {
            nread = 0;
        } else {
            maqueLog(MAQUE_VERBOSE, "Reading from client: %s",strerror(errno));
            freeClient(c);
            return;
        }
    } else if (nread == 0) {
        maqueLog(MAQUE_VERBOSE, "Client closed connection");
        freeClient(c);
        return;
    }
//     if (nread) {
